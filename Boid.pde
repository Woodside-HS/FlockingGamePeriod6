...
class Boid {
  ...
  void update() {
    //update direction
    if(loc.dist(game.base.loc) < 400){
      dir = PVector.sub(loc, game.base.loc);
      dir.normalize();
      dir.mult(.005);
      acc.add(dir);
      acc.mult(0);
  ...
  // Wraparound
  void checkEdges() {
    if (loc.x < r) loc.x = playAreaSize-r;
    if (loc.y < r) loc.y = playAreaSize-r;
    if (loc.x > playAreaSize-r) loc.x = r;
    if (loc.y > playAreaSize-r) loc.y = r;
  }
